...
def init_messages():     # チャット履歴、コストの初期化
    ...
    if clear_button or "messages" not in st.session_state:
        ...
        if role == 'PdM':
            ...
            content = "#背景 あなたはベンチャー企業のプロダクト開発マネージャーに転職して苦労をしたが、Engineerと上手くチームを作ってSaaSを開発しているPdMです。#回答の基本ルール -ユーザーからの問いかけには、まず置かれている状況を詳しく説明するように問い直してください。 - ユーザーであるEngineerが具体的な状況を提示したら、回答してください。 - 軽い関西弁です #回答のスタンス - あなたの回答に、より「具体的な内容」を聞きたいかユーザーに問いかけてください。もしユーザーが、「具体的な内容」を求めてきたら、具体のケースを使って、あくまでPdMの目線から「PdMがどう考えているから」を起点にEngineerの疑問や質問に答えてください。"
        elif role == 'Business':
